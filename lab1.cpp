...
struct graph {
    ...
    template <class T_HC3>
    vector<node<T_HC3>*> Hill_Climbing_Rev3(T_HC3 a, T_HC3 b) {
        node<T_HC3>* ini = find(a);
        node<T_HC3>* fin = find(b);
        ...
        vector<node<T_HC3>*> path;                
        std::unordered_set<node<T_HC3>*> path_set;

        // Funciones auxiliares para mantener sincronizados el vector y el conjunto
        auto push_node = [&](node<T_HC3>* n) {
            path.push_back(n);
            path_set.insert(n);
            };
        auto pop_node = [&]() {
            if (!path.empty()) {
                path_set.erase(path.back()); // Quitar del conjunto
                path.pop_back();             // Quitar del vector
            }
            };
        ...
        auto distance_sq = [](node<T_HC3>* n1, node<T_HC3>* n2) -> float {
        ...
        std::unordered_set<node<T_HC3>*> backtracked_from_nodes; // Nodos de los que retrocedimos

        while (!path.empty() && path.back() != fin) {
            node<T_HC3>* current = path.back();

            if (path.size() > 800) { // Ajusta este límite si es necesario
                cout << "Hill Climbing Rev3 abortado: Camino explorado demasiado largo (>800 nodos)." << endl;
                return {};
            }


            float currentDistSq = distance_sq(current, fin);
            node<T_HC3>* bestNeighbor = nullptr;
            float bestDistSq = currentDistSq;
            node<T_HC3>* sidewaysNeighbor = nullptr;

            for (auto edge : current->edges) {
                node<T_HC3>* neighbor = (edge->nodes[0] == current) ? edge->nodes[1] : edge->nodes[0];
                ...
                if (neighborDistSq < bestDistSq) {
                    ...
                    sidewaysNeighbor = nullptr; // Encontramos uno mejor, descartamos el lateral
                }
                // Comprobar si es un movimiento lateral válido (si no hay uno mejor)
                    ...
                    if (sidewaysNeighbor == nullptr) {
                        sidewaysNeighbor = neighbor; // Guardar el primer candidato lateral
                    }
                }
            } 

            // Tomar decisión: Moverse, moverse lateralmente o retroceder
            if (bestNeighbor) {
                push_node(bestNeighbor);
                backtrackCount = 0;
                sidewaysMovesCount = 0;
                backtracked_from_nodes.clear(); // Limpiar memoria de retroceso al avanzar
            }
                ...
                if (path.size() > 1) {
                    backtracked_from_nodes.insert(current); // Recordar que retrocedimos desde aquí
                    pop_node(); // Quitar nodo actual usando la función auxiliar
                    backtrackCount++;
                    sidewaysMovesCount = 0; // Resetear contador lateral
                }
                else {
